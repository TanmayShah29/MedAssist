...
- [ ] `check_rate_limit` RPC exists: `SELECT proname FROM pg_proc WHERE proname = 'check_rate_limit'`
- [ ] `save_complete_report` RPC exists: `SELECT proname FROM pg_proc WHERE proname = 'save_complete_report'`
...
- [ ] `SUPABASE_SERVICE_ROLE_KEY` only referenced in `src/lib/supabase-admin.ts` (no `"use client"`)
- [ ] CSP header in `next.config.ts` is up to date — includes `https://api.ocr.space` in connect-src
- [ ] `robots.txt` disallows `/dashboard`, `/auth`, `/api/`, `/profile`, `/settings`, `/onboarding`
- [ ] `not-found.tsx` has no `javascript:` protocol hrefs (fixed — uses `onClick` button)
- [ ] Service worker does NOT cache authenticated routes (fixed in `sw.js`)
...
- [ ] `vercel.json` has `maxDuration` set for all AI routes:
  - `analyze-report`: 60s
  - `ask-ai`: 30s
  - `assistant`: 30s
...
- [ ] `/manifest.json` is valid JSON with correct icons

---

## 10. Domain & DNS (first deploy only)
