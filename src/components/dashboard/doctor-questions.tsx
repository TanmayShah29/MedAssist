...
export function DoctorQuestions({ biomarkers, className }: DoctorQuestionsProps) {
  ...
  return (
    <div className={cn("bg-[#FFFFFF] border border-[#EBEAE4] rounded-[18px] p-5 md:p-6", className)}>
      {/* Header */}
      <div className="flex flex-col gap-3 mb-5 sm:flex-row sm:items-start sm:justify-between">
        ...
        {questions.length > 0 && !loading && (
          <button
            onClick={copyAll}
            className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-[8px] text-[12px] font-semibold text-[#475569] bg-white border border-[#EBEAE4] hover:border-sky-300 hover:text-sky-600 active:scale-95 transition-all duration-200 flex-shrink-0 min-h-[36px] self-start"
      ...
      <div className="space-y-3">
        {loading && questions.length === 0 ? (
          ...
          Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-white border border-[#EBEAE4] rounded-[12px] p-4">
          ...
          questions.map((item, idx) => (
            <div
              key={idx}
              className="group bg-white border border-[#EBEAE4] rounded-[12px] p-4 transition-all duration-300 ease-out hover:border-sky-200 hover:shadow-md hover:-translate-y-0.5 stagger-fade-sm"
              ...
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <p className="text-[14px] font-semibold text-[#0F172A] leading-snug break-words min-w-0">{item.question}</p>
                    <button
