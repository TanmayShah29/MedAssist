...
export function ActionItems({ biomarkers }: ActionItemsProps) {
    ...
    return (
        <div className="bg-white rounded-[14px] p-6 border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all duration-300 h-full min-w-0">
            ...
            <div className="space-y-3">
                {discussionItems.length > 0 ? (
                    discussionItems.map((item, idx) => {
                        ...
                        return (
                        <Link key={idx} href={`/assistant?context=${encodeURIComponent(item.name)}`} className="block stagger-fade-sm" style={{ animationDelay: `${idx * 60}ms` }}>
                            <div className={`rounded-lg p-3 border flex gap-3 items-start transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm min-w-0 ${item.status === 'critical' ? 'bg-red-50 border-red-100 hover:bg-red-100' : 'bg-amber-50 border-amber-100 hover:bg-amber-100'}`}>
                                <div className="mt-0.5 bg-white p-1.5 rounded-full shadow-sm shrink-0 transition-transform duration-200 group-hover:scale-110">
                                ...
                                <div className="grow shrink basis-0 min-w-0">
                                    ...
                                    <span className="text-[10px] font-bold uppercase mt-1 inline-flex items-center gap-1">
                                        Prepare talking point <ArrowRight className="w-2.5 h-2.5 transition-transform duration-200 group-hover:translate-x-0.5" />
                                    </span>
                                </div>
                            </div>
                        </Link>
                    ...
                    <div className="bg-emerald-50 rounded-lg p-4 border border-emerald-100 text-center transition-colors duration-300 hover:bg-emerald-100/60">
